...
func ValidateLabel(v *validator.Validator, label *Label) {
	...
	v.Check(label.StoryID != 0, "story", "cannot be empty")
...
func (m LabelModel) Create(label *Label) error {
	query := `INSERT INTO labels (name, story_id)
	VALUES ($1, $2)
	...
	err := m.DB.QueryRowContext(ctx, query, label.Name, label.StoryID).Scan(&label.ID, &label.CreatedAt, &label.Version)
	if err != nil {
		switch {
		case err.Error() == `pq: duplicate key value violates unique constraint "labels_story_id_name_key"`:
...
func (m LabelModel) GetAllLabels(story_id int64) ([]*Label, error) {
	query := `SELECT id, created_at, story_id, name, version
	FROM labels
	WHERE story_id =  $1`
...
func (m LabelModel) GetAllForCharacter(character_id int64) ([]*Label, error) {
	query := `
	SELECT l.id, l.created_at, l.story_id, l.name, l.version
	FROM labels l
